    ...
    <nav className="bg-white/95 backdrop-blur-sm border-b border-stone-100 sticky top-0 z-20">
      <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          {/* Post button — text on sm+, icon on mobile */}
          <Link href="/auth/post"
          ...
          <Link href="/auth/post"
            className="ml-1 w-10 h-10 bg-orange-500 hover:bg-orange-600 text-white rounded-full flex items-center justify-center transition-colors shadow-sm sm:hidden"
            title="New Post">
          ...
          <div className="relative ml-1" ref={profileRef}>
            <button onClick={() => setProfileOpen(p => !p)}
              className="w-10 h-10 rounded-full overflow-hidden border-2 border-transparent hover:border-orange-300 transition-all flex-shrink-0 focus:outline-none">
              {pic ? (
