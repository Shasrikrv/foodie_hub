    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative bg-stone-50 rounded-t-3xl sm:rounded-3xl shadow-2xl w-full sm:max-w-lg max-h-[92vh] flex flex-col">
                ...
                <div key={idx} className="flex flex-col sm:flex-row gap-1.5 sm:gap-2 sm:items-center">
                  <div className="flex gap-2 items-center">
                    ...
                    <input value={ing.name} onChange={(e) => updateIng(idx, "name", e.target.value)}
                      placeholder="Ingredient" className="flex-1 min-w-0 border border-stone-200 rounded-xl px-3 py-2 text-stone-800 text-sm bg-stone-50 focus:outline-none focus:border-orange-400 transition-all" />
                  </div>
                  <div className="flex gap-2 items-center ml-7 sm:ml-0">
                    ...
                    <select value={ing.unit} onChange={(e) => updateIng(idx, "unit", e.target.value)}
                      className="flex-1 sm:flex-none border border-stone-200 rounded-xl px-2 py-2 text-stone-700 text-sm bg-stone-50 focus:outline-none focus:border-orange-400 transition-all">
                      {UNITS.map((u) => <option key={u} value={u}>{u}</option>)}
                    </select>
                    {ingredients.length > 1 && (
                      <button onClick={() => setIngredients((prev) => prev.filter((_, i) => i !== idx))}
                        className="text-stone-300 hover:text-red-400 text-xl font-light transition-colors flex-shrink-0 w-8 h-8 flex items-center justify-center">×</button>
                    )}
                  </div>
                </div>
              ))}
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-stone-200 bg-white rounded-b-3xl flex-shrink-0">
          <button onClick={onClose} className="flex-1 border border-stone-200 text-stone-600 hover:bg-stone-50 font-semibold py-3 rounded-xl text-sm transition-colors">Cancel</button>
...
export default function MyPostsPage() {
  ...
  // Poll open comment sections every 7 seconds for real-time updates
  useEffect(() => {
      ...
      <div className="min-h-screen bg-stone-50 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-3">
            ...
            <p className="text-stone-400 text-sm">Loading your recipes…</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-stone-50 min-h-screen">
      ...
      <div className="max-w-2xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <h1 className="text-xl font-black text-stone-800">My Recipes</h1>
          ...
          <Link
            href="/auth/post"
            className="bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold px-4 py-2 rounded-full transition-colors shadow-sm shadow-orange-100"
          >
            + New Recipe
          </Link>
        </div>
        ...
        {posts.length === 0 ? (
          <div className="text-center py-28">
            ...
            <h3 className="text-stone-700 font-bold text-xl mb-2">No recipes yet</h3>
              ...
              <div className="px-5 pt-3.5 pb-2 flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  {post.visibility === "friends" ? (
                    ...
                    <span className="text-xs font-semibold text-stone-400 bg-stone-50 px-2.5 py-1 rounded-full border border-stone-100">🌍 Everyone</span>
                  )}
                  ...
                  <div className="px-5 pb-4 space-y-2.5">
                    {(comments[post.post_id] || []).length === 0 ? (
                      ...
                      (comments[post.post_id] || []).map((c) => {
                        ...
                        // myPosts API exposes the post author as author_id (not user_id)
